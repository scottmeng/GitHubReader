...
public class ReposWithGraphQLActivity extends AppCompatActivity {
  ...
  private void loadRepos() {
    Request repoRequest = new Request.Builder().url("https://api.github.com/graphql")
        .post(RequestBody.create(MediaType.parse("text/plain"),
            "{\"query\":\"{viewer{repositories(first:30){edges{node{name\\nowner{login}}}}}}\",\"variables\":\"{}\",\"operationName\":null}"))
        .build();

    HttpClient.getClient().newCall(repoRequest).enqueue(new Callback() {
      @Override public void onFailure(Call call, IOException e) {
        Toast.makeText(ReposWithGraphQLActivity.this, "failed", Toast.LENGTH_SHORT).show();
      }
      ...
      @Override public void onResponse(Call call, Response response) throws IOException {
        ...
        try {
          ...
          JSONArray repoEdges = replyObj.getJSONObject("data")
              .getJSONObject("viewer")
              .getJSONObject("repositories")
              .getJSONArray("edges");

          final List<Repo> repos = new ArrayList<>(repoEdges.length());
          for (int i = 0; i < repoEdges.length(); i++) {
            JSONObject nodeObj = repoEdges.getJSONObject(i).getJSONObject("node");
            repos.add(new Repo(nodeObj.getString("name"),
                nodeObj.getJSONObject("owner").getString("login")));
          }
